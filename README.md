...
5. **添加友链**：后台「链接」→ 添加，头像/名称/网址/描述。可创建「链接分类」（工具站/友人等）实现分区展示，分类 slug 用数字前缀（`01-friends`）控制排序
...
友链页面支持按 WordPress 原生链接分类（`link_category`）分区展示：

1. 后台 → 链接 → 链接分类 → 新建分类（填入名称、别名、描述）
2. 编辑链接时指定到对应分类
3. 分类**别名（slug）** 决定展示顺序，使用数字前缀：`01-friends`、`02-tools`
4. 分类**描述**会显示在分类标题下方
